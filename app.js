// ═══════════════════════════════════════
// COACH LION v8 — PRODUCTION READY
// ═══════════════════════════════════════

// ── CONFIG (variables de entorno via Vercel) ──
...
// ── CACHE (memoria + localStorage con TTL) ──
const Cache={
  _m:{},
...
// ── BIOMETRIC (WebAuthn — Face ID / Huella — Android + iOS) ──
const Bio={
  ...
  async register(user){
    ...
    // Pad user ID to exactly 32 bytes
    ...
    localStorage.setItem('cl_bio_id',credId);
    localStorage.setItem('cl_bio_user',user.username);
    return credId;
  },
  async authenticate(){
    if(!await this.isAvailable())throw new Error('Biometría no disponible');
    const credIdStr=localStorage.getItem('cl_bio_id');
    if(!credIdStr)throw new Error('No hay credencial registrada. Actívala en tu perfil.');
...
const DB={
  ...
  async register(name,username,password,profile){
    ...
    const row={id:uid,name,username,password_hash:pwHash,weight:w,height:h,age:profile.age||null,
      ...
      achievements:[],records:{},total_volume:0,privacy_leaderboard:true,created_at:new Date().toISOString()};
    const{error:dbErr}=await sb.from('users').insert(row);
    if(dbErr)throw new Error(dbErr.message);
    Cache.set('user',row);Cache.set('supa_uid',uid);
    return row;
  },
  ...
  async getSession(){
    ...
    if(data.session){const{data:p}=await sb.from('users').select('*').eq('id',data.session.user.id).single();if(p){Cache.set('user',p);return p;}}
  ...
  async getAllUsers(){
    ...
    const r=data||[];Cache.setTTL('all_users',r,5*60*1000);
    return r;
  },
  ...
  async getWorkoutHistory(userId,page=0){
    ...
    const{data}=await sb.from('workout_sessions').select('id,created_at,total_volume,duration_seconds,exercises').eq('user_id',uid).order('created_at',{ascending:false}).range(from,to);
  ...
  async getChallenges(){
    ...
    const r=data||[];Cache.setTTL('challenges',r,5*60*1000);return r;
  },
  ...
  // PHOTOS — Supabase Storage con fallback localStorage
  ...
  async getPhotos(userId){
    ...
    try{
      ...
      if(data?.length){
        return data.map(f=>{const{data:u}=sb.storage.from('progress-photos').getPublicUrl(`${uid}/${f.name}`);return{src:u.publicUrl,date:f.created_at||new Date().toISOString()};});
      }
    }catch(e){}
...
const App={
  ...
  async updateUI(){
    ...
    await this.renderTodayR();this.renderActCal();
  ...
  async finishWorkout(){
    ...
    await DB.saveUser(u);await this.updateUI();S.workLog=[];
  ...
  async renderHistory(page=0){
    ...
    c.innerHTML=hist.map(s=>{
      const d=new Date(s.created_at);const exs=(s.exercises||[]).slice(0,3).map(e=>e.name).join(', ');
      return`<div class="hist-card"><div class="hist-date">${d.toLocaleDateString('es',{weekday:'short',day:'numeric',month:'short'})}</div><div class="hist-body"><div class="hist-vol">📦 ${Math.round(s.total_volume||0).toLocaleString()} kg</div><div class="hist-dur">⏱️ ${pad(Math.floor((s.duration_seconds||0)/60))}:${pad((s.duration_seconds||0)%60)}</div></div>${exs?`<div class="hist-exs">${exs}${(s.exercises||[]).length>3?` +${(s.exercises||[]).length-3} más`:''}</div>`:''}</div>`;
    }).join('');
  ...
  async renderAchs(){
    ...
    $('ach-grid').innerHTML=list.map(a=>{const u=ul.includes(a.id);const imgEl=u&&a.image_data?`<img class="ag-img" src="${a.image_data}" alt="${a.name}">`:`<div class="ag-ico">${u?a.icon:'🔒'}</div>`;return`<div class="ag ${u?'on':'off'}">${u?'<span class="ag-badge">✅</span>':''}${imgEl}<div class="ag-nm">${a.name}</div><div class="ag-desc">${a.description||''}</div><div class="ag-xp" style="color:${RC[a.rarity]}">${RL[a.rarity]} · +${a.xp} XP</div></div>`;}).join('');
  },
  ...
  async renderRecipes(cat){
    ...
    c.innerHTML=list.map(r=>{const macros=[r.calories&&`🔥 ${r.calories} kcal`,r.protein&&`🥩 ${r.protein}g prot`,r.carbs&&`🍚 ${r.carbs}g carbs`].filter(Boolean).join(' · ');return`<div class="recipe-card" onclick="App.openRecipe('${r.id}')">${r.image_data?`<div class="recipe-img"><img src="${r.image_data}" alt="${r.name}"></div>`:`<div class="recipe-img-placeholder">${RECIPE_CAT_L[r.category]||'🥗'}</div>`}<div class="recipe-body"><div class="recipe-cat-badge">${RECIPE_CAT_L[r.category]||r.category}</div><div class="recipe-name">${r.name}</div>${r.description?`<div class="recipe-desc">${r.description}</div>`:''}<div class="recipe-meta">${r.prep_time?`<span class="pill">⏱️ ${r.prep_time} min</span>`:''}<span class="pill" style="color:${r.difficulty==='easy'?'#4caf50':r.difficulty==='medium'?'#f5c518':'#ff6f00'}">${r.difficulty==='easy'?'Fácil':r.difficulty==='medium'?'Moderado':'Difícil'}</span>${(r.tags||[]).map(t=>`<span class="pill">${t}</span>`).join('')}</div>${macros?`<div style="font-size:10px;color:var(--wdim);margin-top:4px">${macros}</div>`:''}</div></div>`;}).join('');
  },
  openRecipe(id){
  ...
  async checkCoachMsg(){
    ...
    try{
      ...
      if(msgs.length>0){const latest=msgs[msgs.length-1];setTimeout(()=>{$('coach-msg-txt').textContent=`"${latest.message}" — ${latest.from_name}`;$('coach-popup')?.classList.remove('hidden');DB.markMessagesRead(S.user.id);},2000);}
    }catch(e){}
  },
