...
const App = {
  ...
  startWorkout(){
    this.goTo('workout');
    if(!S.sessActive){S.sessActive=true;S.sessStart=Date.now();$('sess-tog').textContent='⏸';S.sessTimer=setInterval(()=>this.tickSess(),1000);S.workLog=[];this.renderLog();this.reqWL();}
  },
  tickSess(){if(!S.sessStart)return;const e=Date.now()-S.sessStart;$('sess-t').textContent=`${pad(Math.floor(e/3600000))}:${pad(Math.floor((e%3600000)/60000))}:${pad(Math.floor((e%60000)/1000))}`;},
  toggleSess(){if(S.sessActive){clearInterval(S.sessTimer);S.sessActive=false;$('sess-tog').textContent='▶';}else{S.sessActive=true;S.sessTimer=setInterval(()=>this.tickSess(),1000);$('sess-tog').textContent='⏸';}},
  ...
  finishWorkout(){
    ...
    this.saveUser();S.workLog=[];this.renderLog();$('sess-t').textContent='00:00:00';
